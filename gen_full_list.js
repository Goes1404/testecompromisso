...
function parse(raw) {
    ...
    for(let i=0; i<lines.length; i+=2) {
        ...
        const emailPart = (firstName + secondNameInitial + lastName)
            ...
            .replace(/[^a-z0-9]/g, "");
        res.push({ name, email: `${emailPart}@compromisso.com` });
    }
    return res;
}

const enem = parse(RawDataENEM);
const etec = parse(RawDataETEC);

let md = '# Listagem Completa de Alunos (ENEM & ETEC)\n\n';

md += '## Alunos ENEM (' + enem.length + ')\n\n';
md += '| Nome | E-mail |\n| :--- | :--- |\n';
enem.forEach(s => md += `| ${s.name} | ${s.email} |\n`);

md += '\n## Alunos ETEC (' + etec.length + ')\n\n';
md += '| Nome | E-mail |\n| :--- | :--- |\n';
etec.forEach(s => md += `| ${s.name} | ${s.email} |\n`);

fs.writeFileSync('full_student_list_grouped.md', md);
