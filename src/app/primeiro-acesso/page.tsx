    ...
    try {
      const response = await fetch('/api/student/primeiro-acesso', {
        ...
        body: JSON.stringify({ 
          ...
          institution,
          classroom,
          inviteToken // Passamos o token para validar o cadastro
        })
      });
      const data = await response.json();
        ...
        <div className="w-full max-w-md space-y-10 relative">
          ...
          {step === 'register' && (
            <div className="animate-in fade-in slide-in-from-right-12 duration-700">
              ...
              {!inviteToken && (
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl mb-6 text-amber-700 text-xs font-bold uppercase tracking-wider text-center">
                   Atenção: Cadastro restrito. Use o link oficial de convite.
                </div>
              )}
              ...
              <form onSubmit={handleRegister} className="space-y-6">
                ...
                <Button 
                  ...
                  className={`w-full h-20 text-white rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-2xl transition-all ${!inviteToken ? 'bg-slate-300' : 'bg-orange-600 hover:bg-orange-700 shadow-orange-200'}`}
                >
...
export default function PrimeiroAcessoPage() {
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans selection:bg-indigo-100 selection:text-indigo-900">
      <Suspense fallback={<div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin h-12 w-12 text-indigo-600" /></div>}>
