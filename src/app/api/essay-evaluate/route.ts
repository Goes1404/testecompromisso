...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ── 2. Situações de anulação objetivas, na hierarquia do INEP ──
    // Estas duas não dependem de julgamento: são medidas. Resolver aqui evita
    // gastar três chamadas de IA para concluir o que a contagem já provou, e
    // evita o caso em que o modelo "tem pena" e dá nota a um texto anulável.
    if (analise.textoInsuficiente) {
      return NextResponse.json({
        success: true,
        result: respostaAnulacao(
          'texto_insuficiente',
    ...
    const corrigir = () =>
      openai.chat.completions.create({
        ...
        // dois corretores idênticos não teriam por que discordar, e o
        // protocolo de discrepância não teria função.
        temperature: 0.4,
        max_tokens: 2500,
      });
    ...
    // O INEP só aciona o terceiro nesse caso, e isso economiza uma chamada em
    // toda redação onde os dois primeiros concordam.
    ...
    // ── 5. Anulação por julgamento (fuga ao tema, tipo textual…) ──
    // Exige maioria: um corretor sozinho não anula a redação de ninguém.
    ...
    // ── 6. Protocolo oficial de combinação ──
    ...
    const result = {
      ...
      // Espelho de correção: como a nota foi formada. Vai para o professor na
      // tela de calibração e fica gravado em result_data para auditoria.
      _banca: {
        ...
        usadas: banca.usadas,
        // Sinaliza para a tela do professor que vale uma segunda leitura humana.
